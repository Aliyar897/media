...
def activate_virtualenv():
    ...
    if os.name == 'nt':  # If running on Windows
        ...
        command = f'cmd.exe /c "call {activate_script} && python run_celery_worker.py"'
    else:
        activate_script = os.path.join('venv', 'bin', 'activate')  # Adjust 'venv' to your virtual environment folder
        command = f'bash -c "source {activate_script} && python run_celery_worker.py"'
    
    subprocess.Popen(command, shell=True)

def run_celery_worker():
    command = ['celery', '-A', 'INFomatics', 'worker', '--loglevel=info', '--pool=solo']
